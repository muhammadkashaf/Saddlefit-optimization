...
export const SaddleDetails = () => {
  ...
  useEffect(() => {
    if (klaviyo && data?.saddle) {
      try {
        const saddleData = { ...data?.saddle };
        klaviyo.push(["track", "Viewed Product", saddleData]);
      } catch (error) {
        console.error("Failed to track 'Viewed Product' event in Klaviyo:", error);
      }
    }
  }, [klaviyo, data?.saddle]);
